...
## Version: 
## Last-Updated: jul 22 2025 (09:30) 
##           By: Brice Ozenne
##     Update #: 30
##----------------------------------------------------------------------
## 
...
## *  données de PROFIL  (https://datadryad.org/dataset/doi:10.5061/dryad.c670tq2)
dt.profile <- as.data.table(read.csv("inst/source/PROFIL_DATA.csv", sep ="\t"))
dt.profile[, time := (semaine_patient-1)*7 + jour_semaine]
dt.profile <- dt.profile[,.SD,.SDcols = c("id_total","age","homme","cycle_patient","time","ttt","rcs","nb_crises","duree_tot")]
setnames(dt.profile, old = c("id_total","homme","cycle_patient","ttt","nb_crises","duree_tot"), new = c("id","male","period","treatment","number","duration"))
dt.profile[, treatment := factor(treatment, levels = 1:3, labels = c("placebo","lowDose","highDose"))]
profil <- as.data.frame(dt.profile)
save(profil, file = "data/profil.rda")
